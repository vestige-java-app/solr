...
public class Log {
	...
	/**
	 * Legacy flag indicating if {@link Logger#ignore(Throwable)} methods produce
	 * any output in the {@link Logger}s
	 */
	...
	static {
		...
		// * This is an optional feature used by embedded mode use, and test cases to
		// allow for early
		// * configuration of the Log class in situations where access to the
		// System.properties are
		// * either too late or just impossible.
		loadProperties("jetty-logging.properties", __props);

		// Next see if an OS specific jetty-logging.properties object exists in the
		// classpath.
		// This really for setting up test specific logging behavior based on OS.
		String osName = System.getProperty("os.name");
		if (osName != null && osName.length() > 0) {
			// NOTE: cannot use jetty-util's StringUtil.replace() as it may initialize
			// logging itself.
		...
		while (systemKeyEnum.hasMoreElements()) {
			...
			// Protect against application code insertion of non-String values (returned as
			// null).
			if (val != null)
	...
	public static void initialized() {
		synchronized (Log.class) {
			...
			if (announce && LOG != null)
				LOG.info(
						String.format("Logging initialized @%dms to %s", Uptime.getUptime(), LOG.getClass().getName()));
		}
	}
	 ...
	 * Set the root logger.
	 * <p>
	 * Note that if any classes have statically obtained their logger instance prior
	 * to this call, their Logger will not be affected by this call.
	 *
	 ...
	 * <p>
	 * If there is a different Log class available from a parent classloader, call
	 * {@link #getLogger(String)} on it and construct a {@link LoggerLog} instance
